...
public:
    ...
    void server_init();
    void accept_client();
    void handle_client(std::shared_ptr<Client_> Client);

    void read_get(const std::shared_ptr<Client_> Client);
    void read_user_data(std::shared_ptr<Client_> Client, char header[]);
